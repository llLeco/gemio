...
export class HederaService implements OnModuleInit, OnModuleDestroy {
  ...
  async mintNFT(tokenId: string, metadata: string): Promise<string> {
  ...
  async getMessages(startTime: Date): Promise<string[]> {
    const query: any = new TopicMessageQuery()
      .setTopicId(this.topicId)
      .setStartTime(Timestamp.fromDate(startTime))
      .setEndTime(Timestamp.generate());

    const messages: string[] = [];

    try {
      for await (const message of query.execute(this.client)) {
        messages.push(Buffer.from(message.contents).toString());
      }
    } catch (error) {
